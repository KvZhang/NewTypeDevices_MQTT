...
#ifndef KeyBoardMatrixEvents_H_
#define KeyBoardMatrixEvents_H_
...
class KeyBoardMatrixEvents
{
public:
	KeyBoardMatrixEvents(const uint8_t* linePtr,const uint8_t* columnPtr,uint8_t lineNum,uint8_t columnNum);
	bool keyHandler();//no key be pushed, return 0, need about 80 us
	uint8_t getKey();//will return 0, until all key released
	virtual ~KeyBoardMatrixEvents();
private:
	uint8_t lineNumber;
