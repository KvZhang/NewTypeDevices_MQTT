...
const unsigned int RCSwitch::nSeparationLimit = 4200;
...
//  }
	  ...
	  for(i=1;i<24;i++)
	  {
		  if(RCSwitch::timings[i]>TE_MAX || RCSwitch::timings[i]<TE_MIN )
		  {
			  break;
		  }

	  }
	  if(i<24)//not a HCS301
	  {
		  changeCount = 0;
		  repeatCount = 0;
	  }
	  ...
	  {
		  uint16_t Tte=RCSwitch::timings[24]/10;
		  //just care S0-S3, timing [79,80][S3],[83,84][S0],[85,86][S1],[87,88][S2]
		  uint8_t code=0;
		  code<<=1;
		  if(RCSwitch::timings[147]<(Tte+(Tte>>1)))
		  {
			  code|=1;
		  }
		  code<<=1;
		  if(RCSwitch::timings[149]<(Tte+(Tte>>1)))
		  {
			  code|=1;
		  }
		  code<<=1;
		  if(RCSwitch::timings[151]<(Tte+(Tte>>1)))
		  {
			  code|=1;
		  }
		  code<<=1;
		  if(RCSwitch::timings[145]<(Tte+(Tte>>1)))
		  {
			  code|=1;
		  }
	      RCSwitch::nReceivedValue = code;
	      RCSwitch::nReceivedBitlength = 4;
	      RCSwitch::nReceivedDelay = 10*Tte;
